...
namespace Finsculpt.ViewModels
{
    public partial class HomePageViewModel : ExpensesBaseViewModel
    {
        ...
        public HomePageViewModel(IServiceInterface service)
        {
        ...
        private async Task PrepareAllExpensesChart()
        {
            ...
            var result = LastYearData.GroupBy(d => new { d.DateAdded.Value.Month, d.Name }).Select(g => new { g.Key.Month, g.Key.Name, TotalAmount = g.Sum(d => d.Amount) });
            var groupByMonthNumber = result.GroupBy(d => new { d.Month }).Select(g => new { g.Key.Month, Name = g.Key.Month, TotalAmount = g.Sum(d => d.TotalAmount) });
            ...
            var yearResult = AllYearsData.GroupBy(d => new { d.DateAdded.Value.Year, d.Name }).Select(g => new { g.Key.Year, g.Key.Name, TotalAmount = g.Sum(d => d.Amount) });
            var groupByYearNumber = yearResult.GroupBy(d => new { d.Year }).Select(g => new { g.Key.Year, TotalAmount = g.Sum(d => d.TotalAmount) });
